...
### T-13 AC compressor hardware verification (owner nameplate photo, 2026-09-13)
Owner photographed the compressor label: VW AG **2QD 820 803**, mfr no. **FM10S 14G**, made in China. VAG genuine-parts catalogue titles 2QD 820 803 C "A/c compressor **with electro-magnetic coupling**"; aftermarket cross-refs for the same OE number type it **7VL** (7-cyl variable displacement, Sanden SD7V lineage), 6PK pulley Ø 110-115 mm, fitment India Kushaq/Taigun/Polo. Audit of `AcClimateModel` against this hardware: every numeric assumption SURVIVED (variable-displacement demand-shaped load was already the model form; OFF already priced at 0.0 kW which the clutch confirms; 4.0 kW cap consistent with the 7VL class). Changes made: hardware identity constants (HW_*), corrected comments (clutch is on/off state, not a load term; base kW = minimum-displacement friction with clutch engaged), on-card hardware footnote line, two new guard tests (clutch-open zero floor, hardware identity), and `docs/reference/kylaq-ac-compressor-hardware.md` recording label reading, catalogue cross-refs and modelling consequences. AUTO x0.75 remains a documented assumption pending per-ride learned ON/OFF economy in both AUTO states (evidence rule: measurements beat assumptions).

### T-15 AC/compressor-related PID update audit (owner: "compressor or AC related PIDs were not updating correct")
Audit of every AC-related update path (ambient 0146 -> Temperatures row + AC & Climate card OUT/delta/compressor kW/fuel cost; duplicate research ambient 01BD; compressor state = owner tag, no J1979 PID exists - see docs/reference/kylaq-ac-compressor-hardware.md). Findings:
1. **Priority interval floors (root cause).** 114 of 153 catalogue entries never set `defaultIntervalMs` and inherited the 250 ms constructor default, so every validated SLOW PID - ambient 0146 (the AC card's only live input), trims, tank level, timing - came due on EVERY serial round-robin pass. Consequences: inflated cycle time, wasted ELM327 bandwidth on values that cannot change that fast, higher collision/timeout rates (which then blanked tiles pre-freeze-frame), and lurchy updates across the board including AC tiles. Fix: `PollingPriority.floorMs` (FAST 100 / MEDIUM 400 / SLOW 2000 - the bands already documented on the enum) clamps every interval in ObdScheduler. Guard test scans the whole catalogue.
2. **AC card vs Temperatures row cross-card inconsistency.** The card read the stale-excluded numeric view (flips to "--" during a dropout) while the row kept showing "25 °C (stale)". Fix: pure `numericWithStaleFallback(numeric, decoded)` feeds the card the leading number of a stale-but-valid display string; guard-tested.
3. Already covered by T-8/T-11/T-12: adaptive staleness budgets, freeze-frame on lost frames, and the unit-parse bug that kept AC card OUT at "--" and the AC learning gate dead. 01BD (duplicate ambient research PID) remains in the catalogue but is now floor-clamped like every other SLOW entry.
